...
def search_city(requwest):
    searched_results = []
    city = requwest.GET.get("city")
    if city:
        dest_city_objs = DestinationCityDetails.objects.filter(destination_name__icontains=city)

        for cityfound in dest_city_objs:
            searched_results.append(cityfound.destination_name)
    return JsonResponse({"status":200, "data":searched_results})
